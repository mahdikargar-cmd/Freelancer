...
const ProjectsList: React.FC = () => {
  ...
  const filteredProjects = projects.filter(project => {
    const matchesSearch = project.subject.toLowerCase().includes(searchQuery.toLowerCase()) ||
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full bg-color2 font-primaryMedium border border-color3 rounded-lg py-3 px-4 text-color6 focus:outline-none focus:ring-2 focus:ring-color4 appearance-none"
                >
                  <option value="all" className="font-primaryMedium">همه دسته‌بندی‌ها</option>
                  {categoriesLoading ? (
                    <option disabled>در حال بارگذاری دسته‌بندی‌ها...</option>
                  ) : (
                    getParentCategories().map(parentCategory => (
                      <React.Fragment key={parentCategory.id}>
                        <option value={parentCategory.id.toString()}>
                          {parentCategory.name}
                        </option>
                        {getChildCategories(parentCategory.id).map(childCategory => (
                          <option
                            ...
                            className="pr-4" // Add indentation for child categories
                          >
                            &nbsp;&nbsp;{childCategory.name}
                          </option>
                        ))}
